...
# ---------------- SERIAL READER ----------------
def serial_reader():
    global current_label, header, start_recording, serial_buffer
        ...
        try:
                ...
                try:
                    ...
                    if write_header:
                        with open(CSV_FILE, "a", newline="") as f:
                            ...
                            f.flush()
                            print(f"[CSV] Wrote header to {CSV_FILE}")
                except Exception as e:
                    print("[serial_reader] Error writing header to CSV:", e)
                continue
            ...
            if start_recording:
                ...
                try:
                    ...
                    if "gas_index" in row:
                        row["gas_index"] = int(float(row["gas_index"]))
                    ...
                    if "gas_resistance" in row:
                        row["gas_resistance"] = float(row["gas_resistance"])
                except (ValueError, KeyError):
                    continue

                if "gas_index" in row and row["gas_index"] in [99, 100]:
                    continue
                ...
                with serial_lock:
                    ...
                    # Write row to CSV
                    try:
                        csv_row = [_format_cell(row.get(col, "")) for col in header]
                        with open(CSV_FILE, "a", newline="") as f:
                            writer = csv.writer(f)
                            writer.writerow(csv_row)
                            f.flush()
                    except Exception as e:
                        print("[serial_reader] Error writing CSV row:", e)

                print("[DEBUG] Added row:", ", ".join(f"{k}={v}" for k, v in row.items()))

        except Exception as e:
            print("[serial_reader] Error:", e)

# ---------------- LABEL DASH CALLBACK ----------------
# Label is now set via a Dash input, not input()
def set_label(new_label):
    global current_label
    with serial_lock:
        if new_label and new_label.strip():
            current_label = new_label.strip()
            print(f"[Dash] Current label changed to: {current_label}")
...
def make_figure(df, selected_sensor):
    ...
    if not d.empty:
        ...
        for gi in sorted(d["gas_index"].dropna().unique()):
            ...
            fig.add_trace(go.Scatter(
                x=sub["millis"], y=sub["gas_resistance"], mode="lines+markers",
    ...
    fig.update_layout(template="plotly_dark", hovermode="x unified",
                      height=1100, width=1500, title=f"Sensor: {selected_sensor}")
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div(id="current-label-display", style={"color":"white","marginTop":"10px"})
...
def dash_set_label(n, value):
    if n and value:
        set_label(value)
    return f"Current Label: {current_label if current_label else 'None'}"
...
if __name__ == "__main__":
    current_label = input("Enter initial label: ").strip()
